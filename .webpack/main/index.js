...
/*!*********************!*\
  !*** ./src/main.js ***!
  \*********************/
const { app, BrowserWindow, Menu, BrowserView } = __webpack_require__(/*! electron */ "electron");
...
const createWindow = () => {
  mainWindow = new BrowserWindow({
    ...
    height: 800,
    webPreferences: {
      preload: 'I:\\Koding\\order_history\\order_history_electron\\.webpack\\renderer\\main_window\\preload.js',
    },
  });

  // Головна вкладка (інтерфейс замовлень)
  mainWindow.loadURL('http://localhost:3000/main_window/index.html');

  // Додаємо меню для перемикання вкладок
  const template = [
    {
      label: 'Вкладки',
      submenu: [
...
function openAliExpress() {
  if (!aliExpressView) {
    aliExpressView = new BrowserView({
      webPreferences: {
        nodeIntegration: false,
        contextIsolation: true,
      },
    });
    mainWindow.setBrowserView(aliExpressView);
    aliExpressView.setBounds({ x: 0, y: 0, width: 1200, height: 800 });
    aliExpressView.webContents.loadURL('https://aliexpress.com/');
  }
}

function closeAliExpress() {
  if (aliExpressView) {
    mainWindow.removeBrowserView(aliExpressView);
    aliExpressView = null;
  }
}
